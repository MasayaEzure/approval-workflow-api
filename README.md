...
Spring Boot で「申請・承認ワークフロー API」を実装し、以下を説明できる状態を作る。
...
- Controller: 入出力（DTO 変換）、バリデーション、認可の入口
- Service: 業務ロジック、状態遷移の保証、トランザクション境界
- Repository: 永続化アクセスの抽象化

### Transaction Boundary

状態変更（Request 更新）と履歴追加（RequestHistory/Approval 追加）を同一トランザクションで扱い、
途中失敗時に整合性が崩れないようにする。
...
- システム例外: 想定外の障害  
  業務例外は ControllerAdvice で HTTP レスポンスに変換する。
